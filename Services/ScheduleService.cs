...
public record SemesterOption(string YearId, string YearName, string TermId, string TermName, string DisplayText);
...
public static class ScheduleService
{
    public static async Task<List<JsonElement>> FetchTermsAsync(string studentId, string yearId)
    {
        ...
        if (result.Success && result.Data.TryGetProperty("items", out var items) && items.ValueKind == JsonValueKind.Array)
            return items.EnumerateArray().ToList();
        return [];
    }

    public static async Task<JsonElement?> FetchScheduleAsync(string studentId, string yearId, string termId)
    {
        var result = await ApiService.GetAsync($"/api/student/{studentId}/{yearId}/{termId}/schedule");
    ...
    public static async Task<List<SemesterOption>> LoadAllSemesterOptionsAsync()
    {
        ...
        if (academicYearsElement is not JsonElement yearsEl || yearsEl.ValueKind != JsonValueKind.Array)
            return [];
    ...
    private static async Task EnrichWithYearLevels(List<SemesterOption> options, string studentId)
    {
        ...
        if (studentInfo is not JsonElement info) return;
        ...
        try
        {
            var result = await ApiService.GetAsync($"/api/studentgradefile/student/{studentId}/department/{departmentId}");
            if (!result.Success || !result.Data.TryGetProperty("items", out var items))
                return;

            if (!items.TryGetProperty("studentEnrollments", out var enrollments) || enrollments.ValueKind != JsonValueKind.Array)
                return;
            ...
            for (int i = 0; i < options.Count; i++)
            {
                ...
                {
                    var enrollAcadYear = enrollment.TryGetProperty("academicYear", out var ay) ? ay.GetString() : null;
                    var enrollTerm = enrollment.TryGetProperty("term", out var t) ? t.GetString() : null;

                    if (enrollAcadYear == opt.YearName && enrollTerm == opt.TermName)
                    {
                        var yearLevel = enrollment.TryGetProperty("yearLevel", out var yl) ? yl.GetString() : null;
                        if (yearLevel is not null)
                        {
                            options[i] = opt with { DisplayText = $"{opt.DisplayText} ({yearLevel})" };
                        }
                        break;
                    }
                }
